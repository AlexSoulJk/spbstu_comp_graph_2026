...
private:
    ...
    HRESULT PrefilterCubemapSpecular(ID3D11ShaderResourceView* environmentCubeSRV, UINT prefilterSize, UINT mipLevels, ID3D11ShaderResourceView** outPrefilterSRV);
    HRESULT GenerateBRDFLUT(UINT lutWidth, UINT lutHeight, ID3D11ShaderResourceView** outBRDFLUTSRV);
    HRESULT RebuildIrradianceFromEnvironment();
    HRESULT RebuildSpecularIBLFromEnvironment();
